...
export default function IndicatorRemunerationListClient() {
	...
	const [filters, setFilters] = useState<{
		...
		const facilityTypeId = searchParams.get("facilityTypeId") || "all";
		const indicatorId = searchParams.get("indicatorId") || "all";
	...
	const filtered = useMemo(() => {
		...
		return items.filter((it) => {
			const ft =
				it.facility_type_remuneration?.facility_type?.display_name ||
				it.facility_type_remuneration?.facility_type?.name ||
				"";
	...
	async function loadItems() {
		try {
			...
			const res = await fetch(
				`/api/admin/indicator-remunerations?${params.toString()}`,
				{ cache: "no-store" }
			);
			if (!res.ok) throw new Error(String(res.status));
				...
				<div className="flex flex-col gap-3 md:flex-row md:items-end">
					<div className="w-full md:w-64">
						...
						<Select
							...
							onValueChange={(v) =>
								setFilters((f) => ({ ...f, facilityTypeId: v }))
							}
						>
							<SelectTrigger>
					...
					<div>
						<Button
							...
							onClick={() => {
								...
								router.replace("/admin/indicator-remunerations", {
									scroll: false,
								});
							}}
						>
