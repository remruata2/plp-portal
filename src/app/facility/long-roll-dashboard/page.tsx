...
export default function LongRollDashboardPage() {
  ...
  const loadClinics = async () => {
    try {
      const response = await fetch("/api/facility/long-roll/clinics");
  ...
  const loadStats = async (clinicFilter: string) => {
    try {
      ...
      const url = clinicFilter === "all" 
        ? "/api/facility/long-roll/stats?include_children=true"
        : `/api/facility/long-roll/stats?facility_id=${clinicFilter}`;
