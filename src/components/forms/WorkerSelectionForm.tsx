...
export default function WorkerSelectionForm({
	...
	const [workerAllocationConfig, setWorkerAllocationConfig] = useState<
		Record<
			string,
			{ max_count: number; allocated_amount: number; worker_role: string }
		>
	>({});
	...
	const selectAllWorkers = () => {
		const allWorkerIds = Object.values(workersByType)
			.flat()
			.map((w) => w.id);
					...
					<div className="text-center py-8">
						<div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
							...
							<p className="text-blue-700 text-sm">
								This facility operates on a team-based incentive system. Medical
								Officers receive team-based incentives included in the facility
								total. No individual employee selection is needed.
							</p>
						</div>
					</div>
				</CardContent>
	...
	const totalWorkers = Object.values(workersByType).reduce(
	...
	return (
		<Card style={{ height: "auto", maxHeight: "none", minHeight: "auto" }}>
			<CardHeader>
				<CardTitle className="flex items-center gap-2">
					<Users className="h-5 w-5" />
				...
				{totalWorkers === 0 ? (
					...
					<>
						...
						{(() => {
							...
							const sortedWorkerTypes = workerTypeOrder.filter(
								(type) => workersByType[type] && workersByType[type].length > 0
							);
							...
							return sortedWorkerTypes.map((workerType) => {
								...
								return (
									<div
										key={workerType}
										className={
											sortedWorkerTypes.indexOf(workerType) > 0 ? "mt-6" : ""
										}
									>
