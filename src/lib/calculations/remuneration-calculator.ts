...
import { extractFieldValueForCalculation } from "./formula-calculator/extract-field-value";
...
export class RemunerationCalculator {
	...
	static async calculateFacilityRemuneration(
		...
		try {
			...
			console.log(
				`📊 Facility performance calculated: ${performancePercentage.toFixed(
					2
				)}%`
			);
			...
			if (!facilityTypeRemuneration) {
				throw new Error(
					`No remuneration configuration found for facility type: ${facility.facility_type.name}`
				);
			}
			...
			if (isNaN(facilityIncentive) || facilityIncentive < 0) {
				throw new Error(
					`Invalid facility incentive calculated: ${facilityIncentive}`
				);
			}

			if (isNaN(performancePercentage) || performancePercentage < 0) {
				throw new Error(
					`Invalid performance percentage calculated: ${performancePercentage}`
				);
			}
			...
			console.log(
				`💰 Facility max incentive: ₹${facilityIncentive.toFixed(2)}`
			);
			console.log(
				`💰 Facility actual remuneration: ₹${facilityRemuneration.toFixed(
					2
				)} (${performancePercentage.toFixed(2)}% of max incentive)`
			);
			...
			const individualWorkers = allWorkers.filter(
				(w) => w.worker_type === "hwo" || w.worker_type === "ayush_mo"
			);
			const teamWorkers = allWorkers.filter((w) => w.worker_type === "mo");
			...
			try {
				...
				for (const indicator of indicators) {
					...
					if (!indicatorRemuneration) {
						console.warn(
							`⚠️ No remuneration config found for indicator ${indicator.code}`
						);
						continue;
					}
					...
					if (!submittedValue) {
						console.warn(
							`⚠️ No submitted value found for indicator ${indicator.code}`
						);
						continue;
					}
					...
					console.log(
						`📊 Indicator ${indicator.code}: ₹${incentiveAmount.toFixed(
							2
						)} (${result.achievement.toFixed(1)}%)`
					);
				}

				console.log(
					`💰 Total facility remuneration (sum of indicator incentives): ₹${facilityRemuneration.toFixed(
						2
					)}`
				);
			} catch (error) {
				console.error(
					"Error calculating facility remuneration from indicators:",
					error
				);
				// Fallback to percentage-based calculation
				facilityRemuneration =
					(facilityIncentive * performancePercentage) / 100;
				console.log(
					`💰 Fallback facility remuneration (percentage-based): ₹${facilityRemuneration.toFixed(
						2
					)}`
				);
			}
			...
			if (
				...
				const fallbackAmount =
					(facilityIncentive * performancePercentage) / 100;
				console.log(
					`💡 Applying fallback facility remuneration as ${performancePercentage.toFixed(
						2
					)}% of ₹${facilityIncentive.toFixed(2)} = ₹${fallbackAmount.toFixed(
						2
					)}`
				);
			...
			individualWorkers.forEach((worker) => {
				...
				if (isNaN(allocatedAmount) || allocatedAmount < 0) {
					console.warn(
						`⚠️ Invalid allocated amount for worker ${worker.name}: ${worker.allocated_amount}`
					);
			...
			performanceWorkers.forEach((worker) => {
				...
				if (isNaN(calculatedAmount) || calculatedAmount < 0) {
					console.warn(
						`⚠️ Invalid calculated amount for worker ${worker.name}: ${calculatedAmount}`
					);
					return; // Skip this worker
				}
			...
			console.log(
				`  💰 Personal Incentives: ₹${totalPersonalIncentives.toFixed(2)}`
			);
			console.log(
				`  💰 Facility Remuneration: ₹${facilityRemuneration.toFixed(2)}`
			);
			console.log(
				`  💰 Total Remuneration: ₹${finalCalculation.totalRemuneration.toFixed(
					2
				)}`
			);
	...
	static async getStoredRemunerationCalculation(
		...
		try {
			...
			if (!forceRecalculation) {
				...
				const validation =
					await ConfigurationSnapshot.validateStoredCalculation(
						stored,
						currentVersion
					);
				...
				if (validation.recommendation === "manual_review") {
					console.warn(
						`Version mismatch for ${facilityId} ${reportMonth}: ${validation.differences.join(
							", "
						)}`
					);
				...
				console.log(
					`Recalculating ${facilityId} ${reportMonth} due to version change`
				);
			}
	...
	private static async updateStoredCalculation(
		...
		await prisma.remunerationCalculation.update({
			...
			data: {
				...
				health_workers_count: calculation.workers.filter(
					(w) => w.workerType === "hw"
				).length,
				asha_workers_count: calculation.workers.filter(
					(w) => w.workerType === "asha"
				).length,
	...
	private static async calculatePerformancePercentage(
		...
		try {
			...
			console.log(
				`🔍 Calculating performance for facility: ${facility.name} (${facility.facility_type.name})`
			);
			...
			if (indicators.length === 0) {
				console.log(
					"⚠️ No indicators found for facility type:",
				...
				console.log(
					...
					allIndicators.map((i) => ({
						code: i.code,
						name: i.name,
						facilityTypes: i.applicable_facility_types,
					}))
				);
				return 0;
			}
				...
				console.log(
					`     Target: ${
						indicator.target_value
							? JSON.stringify(indicator.target_value)
							: "None"
					}`
				);
			});
			...
			if (submittedFieldValues && submittedFieldValues.length > 0) {
				...
				console.log(
					`📝 Using ${submittedFieldValues.length} submitted field values`
				);
				...
				fieldValues = basicFieldValues.map((fv) => {
					const fieldWithRelationships = fieldRelationships.find(
						(f) => f.id === fv.field_id
					);
					return {
						...fv,
				...
				console.log(
					`📝 Loaded ${fieldValues.length} field values with relationships`
				);
			}

			if (fieldValues.length === 0) {
				console.log(
					"⚠️ No field values found for facility:",
					facilityId,
					"month:",
					reportMonth
				);
			...
			console.log(
				`📝 Found ${fieldValues.length} field values for facility ${facilityId}, month ${reportMonth}`
			);
			...
			fieldValues.forEach((fieldValue, index) => {
				...
				console.log(
					`     Raw denominator_for_indicators:`,
					denominatorIndicators
				);
				console.log(`     Raw target_for_indicators:`, targetIndicators);

				const hasNumerator =
					numeratorIndicators &&
					Array.isArray(numeratorIndicators) &&
					numeratorIndicators.length > 0;
				const hasDenominator =
					denominatorIndicators &&
					Array.isArray(denominatorIndicators) &&
					denominatorIndicators.length > 0;
				const hasTarget =
					targetIndicators &&
					Array.isArray(targetIndicators) &&
					targetIndicators.length > 0;

				console.log(
					`     Relationships: numerator=${hasNumerator}, denominator=${hasDenominator}, target=${hasTarget}`
				);

				if (hasNumerator) {
					console.log(
						`     Numerator indicators: ${numeratorIndicators
							.map((i: any) => i.code)
							.join(", ")}`
					);
				}
				if (hasDenominator) {
					console.log(
						`     Denominator indicators: ${denominatorIndicators
							.map((i: any) => i.code)
							.join(", ")}`
					);
				}
				if (hasTarget) {
					console.log(
						`     Target indicators: ${targetIndicators
							.map((i: any) => i.code)
							.join(", ")}`
					);
				}
			...
			for (const indicator of indicators) {
				try {
						...
						try {
							...
							const isNumerator =
								...
								numeratorIndicators.some((ind: any) => {
									if (!ind) return false;
									const matches =
										ind.id === indicator.id || ind.code === indicator.code;
									if (matches)
										console.log(
											`       ✅ Found numerator match: ${ind.code} (ID: ${ind.id})`
										);
									return matches;
								});
							...
							const isDenominator =
								...
								denominatorIndicators.some((ind: any) => {
									...
									if (matches)
										console.log(
											`       ✅ Found denominator match: ${ind.code} (ID: ${ind.id})`
										);
									return matches;
								});
							...
							const isTarget =
								...
								targetIndicators.some((ind: any) => {
									...
									if (matches)
										console.log(
											`       ✅ Found target match: ${ind.code} (ID: ${ind.id})`
										);
									return matches;
								});
							...
							if (isNumerator || isDenominator || isTarget) {
								console.log(
									`  ✅ Field ${field.code} (${field.name}) is related to indicator ${indicator.code}`
								);
								return true;
							}
							...
							console.error(
								`  ❌ Error checking field relationships for ${field.code}:`,
								error
							);
							return false;
						}
					...
					if (relevantFieldValues.length === 0) {
						console.log(
							`  ⚠️ No relevant field values found for indicator ${indicator.code}`
						);
					...
					if (indicator.target_value) {
						try {
							...
							if (typeof indicator.target_value === "string") {
								...
								console.log(
									`  ⚠️ Unknown target value format: ${typeof indicator.target_value}`
								);
								targetData = { value: 100 }; // Default fallback
							}

							console.log(
								`  🎯 Raw target value: ${JSON.stringify(
									indicator.target_value
								)}`
							);
							...
							if (
								targetData.min !== undefined &&
								targetData.max !== undefined
							) {
								...
								console.log(
									`  🎯 Using numeric value as target: ${targetValue}`
								);
							} else if (
								typeof targetData === "string" &&
								!isNaN(Number(targetData))
							) {
								targetValue = Number(targetData);
								console.log(
									`  🎯 Converting string to numeric target: ${targetValue}`
								);
							} else {
								console.log(
									`  ⚠️ Could not determine target value from:`,
									targetData
								);
							...
							console.warn(
								`  ⚠️ Error parsing target value for indicator ${indicator.code}:`,
								e
							);
						...
						console.log(
							`  🎯 No target value configured, using default: ${targetValue}`
						);
					}
					...
					for (const fieldValue of relevantFieldValues) {
						if (fieldValue.numeric_value !== null) {
							...
							console.log(
								`  📊 Field ${fieldValue.field.code}: ${value} (max so far: ${actualValue})`
							);
						}
					}
					...
					if (targetValue > 0 && actualValue > 0) {
						indicatorAchievement = Math.min(
							(actualValue / targetValue) * 100,
							100
						);
						console.log(
							`  🎯 Achievement: ${actualValue}/${targetValue} = ${indicatorAchievement.toFixed(
								1
							)}%`
						);
					} else {
						console.log(
							`  ⚠️ Cannot calculate achievement: actual=${actualValue}, target=${targetValue}`
						);
					}
					...
					if (indicatorAchievement > 0) {
						...
						indicatorsWithData++;
						console.log(
							`  ✅ Indicator ${indicator.code}: ${indicatorAchievement.toFixed(
								1
							)}% achievement`
						);
					}
					...
					console.error(
						`❌ Error calculating performance for indicator ${indicator.code}:`,
						error
					);
				}
			}
			...
			if (indicatorsWithData === 0) {
				console.log(
					"❌ No indicators with data found - returning 0% performance"
				);
				return 0;
			}
			...
			console.log(
				`\n🎉 Overall performance: ${performancePercentage.toFixed(
					1
	...
	static async getDistrictRemunerationReport(
		...
		try {
			...
			for (const stored of storedCalculations) {
				...
				if (calculation) {
					const healthWorkers = calculation.workers.filter(
						(w) => w.workerType === "hw"
					).length;
					const ashaWorkers = calculation.workers.filter(
						(w) => w.workerType === "asha"
					).length;
	...
	static async triggerRemunerationCalculation(
		...
		try {
			...
			const currentVersion = await this.getCurrentCalculationVersion();
			const kpiConfig = await ConfigurationSnapshot.getCurrentKPIConfig();
			const remunerationFormula =
				await ConfigurationSnapshot.getCurrentRemunerationFormula();
			const workerAllocationSnapshot =
				await ConfigurationSnapshot.getCurrentWorkerAllocationSnapshot(
					facility.facility_type.id
				);
			const calculationMetadata =
				await ConfigurationSnapshot.getCurrentCalculationMetadata();

			// Store the calculation with configuration snapshots
			console.log(`\n💾 Storing remuneration calculation in database:`);
			console.log(
				`  📊 Performance: ${calculation.performancePercentage.toFixed(2)}%`
			);
			console.log(
				`  💰 Facility Remuneration: ₹${calculation.facilityRemuneration.toFixed(
					2
				)}`
			);
			console.log(
				`  💰 Worker Remuneration: ₹${calculation.totalPersonalIncentives.toFixed(
					2
				)}`
			);
			console.log(
			...
			if (storedCalculation) {
				...
				console.log(
					`  📊 Performance: ${storedCalculation.performance_percentage}%`
				);
				console.log(
					`  💰 Facility Remuneration: ₹${storedCalculation.facility_remuneration}`
				);
				console.log(
					`  💰 Worker Remuneration: ₹${storedCalculation.total_worker_remuneration}`
				);
				console.log(
					`  💰 Total Remuneration: ₹${storedCalculation.total_remuneration}`
				);
			} else {
			...
			try {
				...
				for (const indicator of indicators) {
					try {
							...
							if (denominatorField?.code === "POP001") {
								...
								if (targetType === "BINARY") {
									...
									if (indicator.code === "EC001") {
										...
										denominatorValue =
											clinicTargets[facility.facility_type.name] || 1;
									} else {
						...
						const formulaConfig = (indicator.formula_config as any) || {};
						const maxRemuneration = parseFloat(
						...
						if (!rangeData && indicator.target_value) {
							...
							if (
								targetValueStr.startsWith("{") &&
								targetValueStr.endsWith("}")
							) {
								try {
						...
						if (isNaN(actualValueForDB)) {
							console.warn(
								`⚠️ Invalid actual value for indicator ${indicator.code}: ${actualValue}, skipping storage`
							);
							continue;
						}
							...
							update: {
								...
								target_value: indicator.target_value
									? typeof indicator.target_value === "string"
										? (() => {
												try {
													return JSON.parse(indicator.target_value as any);
												} catch {
						...
						console.log(
							`✅ Stored performance for indicator ${
								indicator.code
							}: ${percentageAchieved.toFixed(
								1
							)}% (${actualValue}/${targetValue})`
						);
					} catch (error) {
						console.error(
							`Error storing performance for indicator ${indicator.code}:`,
							error
						);
				...
				console.log(
					`✅ Stored indicator performance records for facility ${facilityId}, month ${reportMonth}`
				);
			} catch (error) {
			...
			console.log(
				`✅ Stored remuneration calculation, worker records, and indicator performance for facility ${facilityId}, month ${reportMonth}`
			);
		} catch (error) {
