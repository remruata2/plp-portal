...
export class HealthDataRemunerationService {
  ...
  static async processHealthDataRemuneration(
    ...
    try {
        console.log(`🚀 Processing remuneration for facility ${facilityId}, month ${reportMonth}`);
        ...
        console.log(`📍 Processing facility: ${facility.name} (${facility.facility_type.name})`);
        ...
        for (const indicator of indicators) {
          ...
          if (!remuneration) {
            console.log(`⚠️ Skipping indicator ${indicator.code} - no remuneration configuration found`);
            continue;
          }
          ...
          if (indicator.code === "ES001") {
            ...
            console.log(`🔍 Debug ES001: Raw value from fieldValueMap = ${rawValue} (type: ${typeof rawValue})`);
            
            // ES001 is a Yes/No field - form uses "1" for Yes, "0" for No
            if (rawValue === "1" || rawValue === 1 || rawValue === true) {
              actualValue = 1; // "1" = Yes = achieved
              console.log(`🎯 Boolean indicator ${indicator.code}: Value is "1"/Yes, treating as achieved (100%)`);
            } else if (rawValue === "0" || rawValue === 0 || rawValue === false) {
              actualValue = 0; // "0" = No = not achieved
              console.log(`🎯 Boolean indicator ${indicator.code}: Value is "0"/No, treating as not achieved (0%)`);
            } else {
              actualValue = 0; // Default to not achieved for any other value
              console.log(`🎯 Boolean indicator ${indicator.code}: Unexpected value "${rawValue}", treating as not achieved (0%)`);
            }
          }
            ...
            if (targetType === "BINARY") {
              ...
              if (indicator.code === "EC001") {
                ...
                if (indicator.denominator_field_id) {
                  denominatorValue = fieldValueMap.get(indicator.denominator_field_id) || 1;
                  console.log(`🎯 RI sessions indicator ${indicator.code}: Using denominator field value (${denominatorValue}) as denominator`);
                } else {
                  denominatorValue = 1;
                  console.log(`⚠️ No denominator field configured for ${indicator.code}, using 1 as denominator`);
                }
          ...
          try {
            // Calculate remuneration using FormulaCalculator - EXACT SAME AS PERFORMANCE REPORT
            ...
            const result = FormulaCalculator.calculateRemuneration(
              ...
              undefined,
              Object.fromEntries(fieldValueMap) // Pass full field value map like performance report
            );
            
            incentiveAmount = result.remuneration;
          } catch (error) {
            console.error(`Error calculating with FormulaCalculator for indicator ${indicator.code}:`, error);
            incentiveAmount = 0;
          }
          ...
          if (indicator.target_type === "PERCENTAGE_RANGE" && rangeData?.min && rangeData?.max) {
            console.log(`🔍 Debug ${indicator.code}: Range target detected - achievementPercentage: ${achievementPercentage.toFixed(2)}%, rangeData:`, rangeData);
            ...
            console.log(`🔍 Debug ${indicator.code}: Actual percentage = (${actualValue}/${denominatorValue}) * 100 = ${actualPercentage.toFixed(2)}%`);
            ...
            console.log(`🎯 Non-range indicator ${indicator.code}: Capping at 100% - ${achievementPercentage.toFixed(2)}% → ${displayPercentage.toFixed(2)}%`);
          }

          // Convert actualValue to number for database storage (boolean -> number conversion)
          const actualValueForDB = typeof actualValue === 'boolean' ? (actualValue ? 1 : 0) : 
                                   (actualValue !== null && actualValue !== undefined ? Number(actualValue) : undefined);

          // Store in FacilityRemunerationRecord
          try {
            console.log(`💾 Storing FacilityRemunerationRecord for indicator ${indicator.code}:`);
            ...
            const record = await tx.facilityRemunerationRecord.upsert({
              ...
              update: {
                ...
                max_remuneration: parseFloat(remuneration.base_amount.toString()),
                status: displayPercentage && displayPercentage >= 100 ? "achieved" :
                        displayPercentage && displayPercentage >= 50 ? "partial" : "not_achieved",
            ...
            console.log(`✅ Successfully stored FacilityRemunerationRecord for indicator ${indicator.code}`);
            indicatorRecords.push(record);
          } catch (error) {
            console.error(`❌ Error storing FacilityRemunerationRecord for indicator ${indicator.code}:`, error);
            // Continue with other indicators instead of failing the entire transaction
          }
        }

        console.log(`✅ Stored ${indicatorRecords.length} indicator performance records`);
        ...
        console.log(`💰 Facility remuneration (sum of indicator incentives): ₹${facilityRemuneration.toFixed(2)}`);
        ...
        console.log(`🔍 DEBUG SERVICE: Calculating overall performance percentage:`);
        console.log(`🔍 DEBUG SERVICE: Total indicators in calculation: ${indicatorRecords.length}`);
        ...
        indicatorRecords.forEach((record, index) => {
          ...
          console.log(`🔍 DEBUG SERVICE: ${index + 1}. ${record.indicator_code}: ${originalPercentage.toFixed(2)}% → ${cappedPercentage.toFixed(2)}% (capped)`);
          totalPercentage += cappedPercentage;
        });
        const performancePercentage = totalPercentage / indicatorRecords.length;
        
        console.log(`🔍 DEBUG SERVICE: Total percentage sum: ${totalPercentage.toFixed(2)}%`);
        console.log(`🔍 DEBUG SERVICE: Number of indicators: ${indicatorRecords.length}`);
        console.log(`🔍 DEBUG SERVICE: Final performance percentage: ${performancePercentage.toFixed(2)}% (${totalPercentage.toFixed(2)} / ${indicatorRecords.length})`);
        console.log(`🔍 DEBUG SERVICE: Manual verification: ${totalPercentage.toFixed(2)} ÷ ${indicatorRecords.length} = ${(totalPercentage / indicatorRecords.length).toFixed(2)}%`);
        ...
        for (const worker of healthWorkers) {
          ...
          if (workerType === 'hw' || workerType === 'asha') {
            ...
            const allocatedAmount = parseFloat(worker.allocated_amount.toString());
            const workerIncentive = (allocatedAmount * performancePercentage) / 100;
            
            totalWorkerRemuneration += workerIncentive;

            // Store in WorkerRemuneration table
            console.log(`💾 Storing worker remuneration for worker ${worker.id}:`, {
              worker_type: worker.worker_type,
              worker_role: (worker.worker_type || 'UNKNOWN').substring(0, 50).toUpperCase(),
              facility_id: facilityId,
            ...
            const workerRecord = await tx.workerRemuneration.upsert({
              ...
              update: {
                ...
                worker_role: (worker.worker_type || 'UNKNOWN').substring(0, 50).toUpperCase(), // Truncate to prevent column length issues
        ...
        console.log(`✅ Stored ${workerRecords.length} worker remuneration records`);
        ...
        const remunerationCalculation = await tx.remunerationCalculation.upsert({
          ...
          update: {
            ...
            health_workers_count: healthWorkers.filter(w => w.worker_type === 'hw').length,
            asha_workers_count: healthWorkers.filter(w => w.worker_type === 'asha').length,
            calculated_at: new Date(),
          },
          create: {
            facility_id: facilityId,
        ...
        console.log(`📊 Performance: ${remunerationCalculation.performance_percentage}%`);
        console.log(`💰 Facility Remuneration: ₹${facilityRemuneration.toFixed(2)}`);
        console.log(`💰 Worker Remuneration: ₹${totalWorkerRemuneration.toFixed(2)}`);
        console.log(`💰 Total Remuneration: ₹${remunerationCalculation.total_remuneration.toFixed(2)}`);

        const healthWorkersCount = healthWorkers.filter((w: any) => w.worker_type === 'hw').length;
        const ashaWorkersCount = healthWorkers.filter((w: any) => w.worker_type === 'asha').length;
