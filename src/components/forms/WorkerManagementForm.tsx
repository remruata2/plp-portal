		...
		const loadData = async () => {
			try {
				...
				if (workersResponse.ok && configResponse.ok) {
					...
					const validWorkers = ((workersData.workers as Worker[]) || []).filter(
						(w: Worker) => w.name.trim()
					);
					...
					if (configData.config) {
						const configArray = Object.entries(
							configData.config as Record<string, WorkerAllocationDetails>
						).map(([worker_type, details]) => ({
							worker_type,
		...
		try {
			...
			toast.error(
				error instanceof Error
					? error.message
					: "Failed to save facility employees"
			);
		} finally {
			setSaving(false);
		}
							...
							<div className="flex items-center justify-between">
								<div>
									...
									<p className="text-sm text-gray-500 mt-1">
										Allocation: ₹{config.allocated_amount.toLocaleString()}
										/month
									</p>
								</div>
								<Button
									...
									return (
										<div key={globalIndex} className="flex items-center gap-2">
											<Input
												...
												value={worker.name}
												onChange={(e) =>
													updateWorker(globalIndex, e.target.value)
												}
												className="flex-1"
											/>
											<Button
