...
export default function SubmissionDeadlineGuard({
	...
	const [submissionStatus, setSubmissionStatus] =
		useState<SubmissionDeadlineStatus | null>(null);
	...
	// Admins can always bypass deadline restrictions
	const isAdmin = session?.user?.role === "admin";

	// If submissions are allowed OR user is admin, render the children
	if (submissionStatus?.isSubmissionAllowed || isAdmin) {
		return <>{children}</>;
	}
		...
		<Card className={`border-red-200 ${className}`}>
			...
			<CardContent className="space-y-4">
				<Alert variant="destructive">
					...
					<AlertDescription>
						<strong>Data submissions are currently closed.</strong> The deadline
						for this month was {submissionStatus?.deadlineDay}th, and facilities
						can no longer submit new data.
					</AlertDescription>
				</Alert>
						...
						<div className="grid grid-cols-2 gap-4 text-sm">
							<div>
								...
								<span className="ml-2 font-medium">
									{submissionStatus.currentDay}
								</span>
							</div>
							<div>
								<span className="text-gray-600">Deadline Day:</span>
								<span className="ml-2 font-medium">
									{submissionStatus.deadlineDay}
								</span>
							</div>
							<div>
