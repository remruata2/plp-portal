...
export default function AdminPerformanceReportsPage() {
	...
	// Modal states
	const [selectedReport, setSelectedReport] = useState<FacilityReport | null>(
		null
	);
	...
	const handleViewReport = (report: FacilityReport) => {
		...
		router.push(
			`/admin/performance-reports/${report.facility.id}/${report.reportMonth}`
		);
	};
	...
	const handleSaveIncentives = async () => {
		...
		try {
			...
			const response = await fetch(
				"/api/admin/performance-reports/update-incentives",
				{
					method: "POST",
					headers: {
						...
						<div>
							...
							>
								<SelectTrigger>
									<SelectValue
										placeholder={
											availableYears.length ? "Select month" : "No months"
										}
									/>
								</SelectTrigger>
								<SelectContent>
									{availableMonths
										...
										.map((month) => (
											<SelectItem key={month} value={month}>
												{new Date(`2000-${month}-01`).toLocaleString("en-US", {
													month: "long",
												})}
											</SelectItem>
										))}
			...
			{loading ? (
				...
				<div className="grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 gap-6">
					{filteredReports.map((report) => (
						<Card
							key={`${report.facility.id}-${report.reportMonth}`}
								...
								<div className="space-y-4">
									...
									<div className="grid grid-cols-2 gap-4 text-sm">
										<div className="flex items-center gap-2">
											...
											<span>
												Achieved: {report.summary.achievedIndicators}/
										...
										<div className="flex items-center gap-2">
											...
											<span>{report.workers?.length || 0} Workers</span>
										</div>
										<div className="flex items-center gap-2">
									...
									<div className="space-y-2">
										...
										<div className="flex gap-1">
											...
											<div
												...
												style={{
													width: `${
														(report.summary.partialIndicators /
															report.summary.totalIndicators) *
														100
													}%`,
												}}
											...
											<div
												...
												style={{
													width: `${
														(report.summary.notAchievedIndicators /
															report.summary.totalIndicators) *
														100
													}%`,
												}}
				...
				<Card>
					<CardContent className="p-12">
						<div className="text-center text-gray-500">
							...
							<p className="text-sm">
								Try adjusting your filters or check if data has been submitted
			...
			<Dialog open={isEditModalOpen} onOpenChange={setIsEditModalOpen}>
				<DialogContent className="max-w-2xl max-h-[80vh] overflow-y-auto">
					<DialogHeader>
						...
						<DialogDescription>
							Manually adjust worker incentive amounts for{" "}
							{selectedReport?.facility.display_name}
						</DialogDescription>
					</DialogHeader>
						...
						{editingWorkers.map((worker) => (
							<div key={worker.id} className="border rounded-lg p-4">
								...
								<div className="grid grid-cols-2 gap-4">
									...
									<div>
										<Label htmlFor={`awarded-${worker.id}`}>
											Awarded Amount
										</Label>
										<Input
											...
											onChange={(e) =>
												handleWorkerAmountChange(worker.id, e.target.value)
											}
										/>
									</div>
								</div>
