...
export default function FacilityWorkersPage() {
	...
	useEffect(() => {
		const loadData = async () => {
			try {
				...
				const [facilityResponse, workersResponse, configResponse] =
					await Promise.all([
						fetch(`/api/facilities/${session.user.facility_id}`),
						fetch(`/api/facilities/${session.user.facility_id}/workers`),
						fetch(
							`/api/facilities/${session.user.facility_id}/worker-allocation-config`
						),
					]);
				...
				if (configData.config) {
					const configArray = Object.entries(configData.config).map(
						([worker_type, details]: [string, any]) => ({
							worker_type,
	...
	useEffect(() => {
		...
		if (filterWorkerType !== "all") {
			filtered = filtered.filter(
				(worker) => worker.worker_type === filterWorkerType
			);
		}
	...
	const refreshWorkers = async () => {
		...
		try {
			const response = await fetch(
				`/api/facilities/${session.user.facility_id}/workers`
			);
			if (response.ok) {
	...
	const handleSaveWorker = async () => {
		...
		try {
			...
			if (response.ok) {
				toast.success(
					selectedWorker
						? "Facility Employee updated successfully!"
						: "Facility Employee added successfully!"
				);
				await refreshWorkers();
			...
			toast.error(
				error instanceof Error
					? error.message
					: "Failed to save Facility Employee"
			);
		} finally {
			setSaving(false);
		}
	...
	return (
		<div className="max-w-7xl mx-auto p-6 space-y-6">
			...
			{(facility?.facility_type.name === "UPHC" ||
				facility?.facility_type.name === "U_HWC") && (
						...
						<div className="flex flex-col sm:flex-row sm:items-start gap-3">
							...
							<div className="flex-1 min-w-0">
								...
								<p className="text-sm text-blue-700 leading-relaxed">
									This facility operates on a team-based incentive system. Only
									Medical Officers (MO) can be added. No individual employee
					...
					<div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
						...
						<div className="space-y-2">
							...
							<Select
								...
								onValueChange={setFilterWorkerType}
							>
								<SelectTrigger>
									<SelectValue />
								</SelectTrigger>
											...
											<TableCell>
												<div className="text-sm">
													{worker.contact_number && (
														<div>{worker.contact_number}</div>
													)}
													{worker.email && (
														<div className="text-gray-500">{worker.email}</div>
													)}
													{!worker.contact_number && !worker.email && (
			...
			<Card>
				...
				<CardContent>
					<div className="space-y-2 text-sm text-gray-600">
						<p>
							• Add facility employees who will receive performance-based
							incentives
						</p>
						<p>
							• You can add multiple employees of each type based on your
							facility's allocation
						</p>
						<p>
							• These employees will be available for selection during monthly
							data submission
						</p>
						<p>
							• You can edit employee details or remove employees at any time
						</p>
						<p>
							• Employee allocation amounts are automatically set based on your
							facility type
						</p>
					</div>
				</CardContent>
					...
					<div className="space-y-4">
						...
						<div>
							...
							<Select
								...
								onValueChange={(value) =>
									setFormData({ ...formData, worker_type: value })
								}
							>
								<SelectTrigger>
								...
								<SelectContent>
									{allocationConfig
										...
										.map((config) => {
											const currentCount = workers.filter(
												(w) =>
													w.worker_type === config.worker_type && w.is_active
											).length;
			...
			>
				<AlertDialogContent>
					<AlertDialogHeader>
						...
						<AlertDialogDescription>
							Are you sure you want to delete "{selectedWorker?.name}"? This
