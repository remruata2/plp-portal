...
export async function GET(request: NextRequest) {
  try {
    ...
    if (requestedFacilityId) {
      ...
      // All clinics - get all facilities with has_clinic=true
      const allClinics = await prisma.facility.findMany({
        where: {
          has_clinic: true,
          is_active: true,
        },
      ...
      facilityIds = allClinics.map(c => c.id);
      facility_id = allClinics[0]?.id || ""; // Use first clinic for display info
    }

    if (!facility_id || facilityIds.length === 0) {
