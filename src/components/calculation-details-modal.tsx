...
export function CalculationDetailsModal({
	...
	const getFormulaDisplay = () => {
		...
		const denominatorFieldName =
			indicator.denominator_field?.name || "Denominator";
		...
		const getCalculationDisplay = () => {
			if (formula === "(A/(B/12))*100") {
				...
				return `${numeratorValue} ÷ (${denominatorValue} ÷ 12) × 100 = ${percentage.toFixed(
					1
				)}%`;
			} else if (formula === "(A/(B/60))*100") {
				// Population-based formula: A/(B/60)*100
				return `${numeratorValue} ÷ (${denominatorValue} ÷ 60) × 100 = ${percentage.toFixed(
					1
				)}%`;
			} else {
				// Standard formula: A/B*100
				return `${numeratorValue} ÷ ${denominatorValue} × 100 = ${percentage.toFixed(
					1
				)}%`;
			}
		};
		...
		switch (targetType) {
			case "PERCENTAGE_RANGE":
				return {
					...
					explanation:
						...
						"Percentage calculation based on numerator and denominator",
					values: {
						...
						"Formula Description":
							formulaConfig.description || "Standard percentage calculation",
					},
				};
			...
			case "RANGE":
				return {
					...
					explanation:
						"Achievement scales linearly from minimum to maximum target",
					values: {
						"Actual Value": `${actual}`,
						"Target Range": indicator.target_description || "Standard range",
						Formula: `${formula}`,
			...
			case "BINARY":
				return {
					...
					values: {
						...
						Calculation: getCalculationDisplay(),
						Result:
							percentage >= 100 ? "Target achieved" : "Target not achieved",
					},
				};
	...
	const getIncentiveCalculation = () => {
		const percentage = indicator.raw_percentage || indicator.percentage || 0;
		const maxIncentive =
			indicator.max_remuneration || indicator.incentive_amount;
	...
	return (
		<>
			<Button
				variant="outline"
				size="sm"
				className="flex items-center justify-center gap-2 text-xs min-w-[100px] whitespace-nowrap"
				type="button"
									...
									<div className="space-y-2 text-sm">
										<div className="flex justify-between">
											...
											<span className="font-mono text-blue-800">
												{indicator.formula_config.calculationFormula ||
													"(A/B)*100"}
											</span>
										</div>
							...
							<div className="bg-yellow-50 p-4 rounded-lg border border-yellow-200">
								...
								<div className="space-y-2 text-sm text-yellow-800">
									<div className="font-mono bg-white p-2 rounded border">
										...
										<div className="ml-4">
											{indicator.numerator_field?.name || "Numerator"} (A) ={" "}
											{indicator.numerator_value || indicator.actual}
										</div>
										<div className="ml-4">
											{indicator.denominator_field?.name || "Denominator"} (B) ={" "}
											{indicator.denominator_value ??
												indicator.target_value_for_calculation ??
												1}
										</div>
									...
									<div className="font-mono bg-white p-2 rounded border">
										...
										<div className="ml-4">
											{(() => {
												...
												if (formula === "(A/(B/12))*100") {
													// Population-based formula: A/(B/12)*100
													return `${numerator} ÷ (${denominator} ÷ 12) × 100 = ${
														indicator.raw_percentage?.toFixed(2) ||
														indicator.percentage?.toFixed(1)
													}%`;
												} else if (formula === "(A/(B/60))*100") {
													// Population-based formula: A/(B/60)*100
													return `${numerator} ÷ (${denominator} ÷ 60) × 100 = ${
														indicator.raw_percentage?.toFixed(2) ||
														indicator.percentage?.toFixed(1)
													}%`;
												} else {
						...
						<div className="bg-gray-50 p-4 rounded-lg">
							<div className="flex items-start gap-2">
								...
								<div className="text-sm text-gray-600">
									...
									<p>
										This indicator contributes to your overall facility
										performance score, which determines the distribution of
										incentives across your health employees. Higher achievement
										percentages result in better incentive distribution.
									</p>
								</div>
							</div>
						</div>
